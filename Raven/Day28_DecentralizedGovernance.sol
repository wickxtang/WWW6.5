...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    uint256 public quorumPercentage;// Allowed proposal passing percentage
	...
	// User can create proposal with deposit
	...
	// User can vote for proposal according to their token amount
	function vote(uint256 proposalId, bool support) external {
		Proposal storage proposal = proposals[proposalId];
		require(block.timestamp >= proposal.startTime && block.timestamp <= proposal.endTime, "Not in time");
		require(!proposal.executed, "Executed");
		require(!proposal.canceled, "Canceled");
		require(!hasVoted[proposalId][msg.sender], "Already voted");
		uint256 weight = governanceToken.balanceOf(msg.sender);
		require(weight > 0, "Invalid amount");
	...
	// Finalize proposal after some time
	...
	// Execute proposal after some lock time
	// Return deposit to proposer if proposal passes
